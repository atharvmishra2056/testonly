...
class VerifyView(View):
    ...
    @discord.ui.button(label="✅ Verify Now",
                       style=discord.ButtonStyle.green,
                       custom_id="verify_kuzz_v5")
    async def verify_button(self, button, interaction):
        await interaction.response.defer(ephemeral=True)
        member = interaction.user
        current_time = discord.utils.utcnow().timestamp()
        if button_cooldowns.get(member.id,
                                0) + COOLDOWN_SECONDS > current_time:
            await interaction.followup.send("Please wait before trying again.",
                                            ephemeral=True)
            return
        ...
        embed = discord.Embed(
            ...
            description=
            f"Enter code: **{captcha_code}** (within {CAPTCHA_TIMEOUT}s)",
        ...
        try:
            ...
            if role and role not in member.roles:
                ...
                await verify_channel.set_permissions(member,
                                                     view_channel=False)
                welcome_embed = discord.Embed(
                    title="🎉 Welcome to KuzzMarket!",
                    description=
                    f"Congrats, {member.mention}! You are our **{len(interaction.guild.members)}th member**! Join #💬 | General.",
                    color=discord.Color.blue())
                await interaction.followup.send(embed=welcome_embed,
                                                ephemeral=True)
                log_channel = bot.get_channel(LOG_CHANNEL_ID)
                if log_channel:
                    log_embed = discord.Embed(title="Verification Log",
                                              color=discord.Color.purple())
                    log_embed.add_field(name="User",
                                        value=member.mention,
                                        inline=True)
                    log_embed.add_field(name="Role",
                                        value=role.name,
                                        inline=True)
                ...
                await interaction.followup.send(
                    "Already verified or role missing!", ephemeral=True)
        except asyncio.TimeoutError:
            await interaction.followup.send("CAPTCHA timed out. Try again.",
                                            ephemeral=True)
        finally:
...
class RoleSelect(Select):
    ...
    def __init__(self):
        options = [
            discord.SelectOption(label="Buyer",
                                 description="Access to buy services",
                                 custom_id="role_select_buyer"),
            discord.SelectOption(label="Seller",
                                 description="Sell your services",
                                 custom_id="role_select_seller")
        ]
        super().__init__(placeholder="Choose a role...",
                         min_values=1,
                         max_values=1,
                         options=options,
                         custom_id="role_select_kuzzmarket")
    ...
    async def callback(self, interaction):
        ...
        if role not in interaction.user.roles:
            ...
            await interaction.response.send_message(
                f"Assigned {role_name} role!", ephemeral=True)
        else:
            await interaction.response.send_message(
...
async def on_ready():
    ...
    if channel:
        embed = discord.Embed(
            ...
            description="Welcome! Click to verify and join KuzzMarket.",
...
async def on_message(message):
    ...
    if xp[user_id] >= 100:
        ...
        if not role:
            role = await message.guild.create_role(name="Active Member",
                                                   color=discord.Color.gold())
        if role and role not in message.author.roles:
            await message.author.add_roles(role)
            await message.channel.send(
                f"{message.author.mention} is now an Active Member!")
        xp[user_id] = 0
...
@bot.slash_command(name="createrole",
                   description="Create a custom role (Mods only)",
                   guild_ids=[GUILD_ID])
...
async def createrole(ctx, name: str, color: str = "0x000000"):
    try:
        ...
        await ctx.respond(f"Created {role.name} with color {color}!",
                          ephemeral=True)
    except ValueError:
...
@bot.slash_command(name="services",
                   description="View marketplace services",
                   guild_ids=[GUILD_ID])
async def services(ctx):
    embed = discord.Embed(title="🛒 KuzzMarket Services",
                          description="Professional growth services!",
                          color=discord.Color.gold())
    embed.add_field(name="Instagram Likes",
                    value="500 Likes - $5",
                    inline=True)
    embed.add_field(name="YouTube Subs", value="100 Subs - $10", inline=True)
    embed.add_field(name="Custom",
                    value="Contact Sellers in #🛒 | Discord",
                    inline=True)
    await ctx.respond(embed=embed, view=MarketplaceView())


@bot.slash_command(name="sendverify",
                   description="Send verification message (Admins only)",
                   guild_ids=[GUILD_ID])
...
@bot.slash_command(name="clearverify",
                   description="Clear old verify messages (Admins only)",
                   guild_ids=[GUILD_ID])
@commands.has_permissions(administrator=True)
